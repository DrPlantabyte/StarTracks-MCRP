...
execute as @a at @s if score @s _st_briefing matches 30 run tellraw @s ["",{"text":"[Chief Scientist Alex] ","color":"green"},{"text":"Without the Ender Dragon, the cyborg threat has been eliminated within 1 kilometer of the outpost. Lands farther out may still be dangerous.'","color":"white"}]
...
execute as @a at @s if score @s _st_briefing matches 310.. run function startracks:missions/m32_ender1e_02main_start