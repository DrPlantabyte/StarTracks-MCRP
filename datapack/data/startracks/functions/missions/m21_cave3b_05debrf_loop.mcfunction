...
execute as @a at @s if score @s _st_briefing matches 190 run tellraw @s ["",{"text":"[Chief Scientist Alex] ","color":"green"},{"text":"Wait, I'm picking up a strange signal from deep underground... Commander Steve, you need to see this!","color":"white"}]
execute as @a at @s if score @s _st_briefing matches 330.. run function startracks:missions/m21_cave3b_06end